...
def format_original_content_summary(original_content: str, original_text_content: str) -> str:
    """
    格式化原始内容和文本内容为指定格式的字符串
    
    Args:
        original_content: 原始内容字符串
        original_text_content: 原文内容字符串
    
    Returns:
        格式化后的字符串，格式为：
        "总结": original_content
        "原文": original_text_content
    """
...
def format_sources_with_hover(sources: List[Dict], answer_text: str) -> Tuple[str, Dict, Dict]:
    ...
    for old_idx in citation_order:
        if old_idx <= len(sources):
            ...
            article_key = f"{source_info}|{year}"
            original_content = metadata.get('llm_text', '') or source.get('content', '')
            original_text_content = metadata.get('original_text', '')
            ...
            individual_hover_data[old_idx] = {
                'source': source_info,
                'year': year,
                'content':  format_original_content_summary(original_content,original_text_content)
            }
...
def add_citation_tooltips(text: str, hover_data: Dict, citation_remap: Dict, msg_id: str) -> str:
    ...
    def _replace_bracket_citation(match):
        ...
        # )   
        return (
            f'<a class="citation-link" href=" "><sup>[{new}]</sup></a >'
            f'<span class="citation-payload" id="{cid}">{b64}</span>'
        )
